...
| Regression | factor/dummy paths | Factor helpers are ported, but regression factor dispatch is not wired | Enable factor expansion in `nns_reg` classification paths |
...
| Multivariate regression | `factor_2_dummy=True` | Factor helpers and class distance are ported, but multivariate factor dispatch is not wired | Enable factor expansion in `nns_m_reg` classification paths |
