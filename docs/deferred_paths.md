...
| Regression | `factor_2_dummy=True` with `dim_red_method` | Public factor predictors and numeric dimension reduction are supported separately, but R's factor-expanded dimension-reduction projection path still needs parity mapping | Probe installed R with factor predictors plus each dim-red method, then port the projection/diagnostic conventions |
