...
| `NNS.nowcast` core | `nns_nowcast_panel`, `nns_nowcast(fetch=True, provider_backend=...)`, `pynns.providers.CsvNowcastProvider`, `pynns.providers.FredApiNowcastProvider` | Deterministic user-supplied monthly panel and explicit provider path, including local CSV panels and optional FRED API panels; no default live provider |
